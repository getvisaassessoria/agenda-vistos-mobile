...
def salvar():
    ...
    try:
        ...
        for atividade, data, hora in etapas:
            # CORREÇÃO CRÍTICA: Converte DD/MM/YYYY para YYYY-MM-DD antes de salvar
            data_banco = datetime.strptime(data, "%d/%m/%Y").strftime("%Y-%m-%d")
            cursor.execute(
                "INSERT INTO compromissos (cliente, atividade, data, hora, local) VALUES (%s, %s, %s, %s, %s)",
                (cliente_final, atividade, data_banco, hora, local)
            )
        conn.commit()
        conn.close()
        ...
        messagebox.showinfo("Sucesso", "Compromissos salvos com sucesso!")
    except Exception as e:
...
def ver_agenda():
    ...
    def editar_selecionado():
        ...
        valores = tree.item(selecionado[0], "values")
        if valores[0] == "": return # Ignora se clicar na linha separadora
        ...
        def salvar_edicao():
            ...
            try:
                # Converte para o padrão invisível do banco antes de salvar
                ...
                janela_edit.destroy()
                carregar_dados() # Atualiza a tabela na hora
            except Exception as e:
